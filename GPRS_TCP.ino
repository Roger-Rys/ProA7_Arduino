bool TCP_GPRS() { //voil TCP_GPRS()
  ...
  delay(100);
  escribirComando("AT+CGATT=1", 2000, true);
  delay(100);
  ...
  delay(100);
  escribirComando("AT+CGACT=1,1", 5000, true); //Activate PDP, open Internet service
  delay(100);
  bool confirmar = escribirComando("AT+CIPSTART=TCP,api.thingspeak.com,80", 8000, true);
  if(confirmar){
    Serial.print("confirmar_CIPSTART:");Serial.println(confirmar);
    return true; 
  }
  return false;   
}
